...
serve(async (req) => {
  ...
  try {
    ...
    const { clientEmail, newPassword } = await req.json();
    
    if (!clientEmail || !newPassword) {
      return new Response(
        JSON.stringify({ error: "Se requiere email del cliente y nueva contraseña" }),
        { 
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400
        }
    ...
    const clientUser = userData.users.find(u => u.email === clientEmail);
    if (!clientUser) {
      return new Response(
